...
//add
class Employee {
    constructor(id, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, SALARY, DEPARTMENT_ID, Image) {
        this.id = id;
...
function getEmployeeObject() {
    ...
    const PHONE_NUMBER = prompt("Enter Your Phone number: ");
    const HIRE_DATE = prompt("Enter Your HirDate: ");
    const SALARY = prompt("Enter Your Salary: ");
    const DEPARTMENT_ID = prompt("Enter Your First name: ");
    const Image = prompt("Enter Your First name: ");

    return new Employee(id, FIRST_NAME, LAST_NAME, EMAIL, PHONE_NUMBER, HIRE_DATE, SALARY, DEPARTMENT_ID, Image);

}

function addEmployee(){
    let newEmployee = getEmployeeObject();
...
function readData() {
    ...
    console.log("before delete = " + employeesData.length);
...
//Search or filter
//Find the employee Name having dept id - 30
function searchByDeptId() {

    console.log(employeesData.filter(employee => employee.DEPARTMENT_ID == 30).map(employee => employee.FIRST_NAME + " " + employee.LAST_NAME + "(Dept Id) - " + employee.DEPARTMENT_ID));
}
// searchByDeptId();
//find the employees details joined after year 1994
function searchByjoiningYear() {

    console.log(employeesData.filter(employee => new Date(employee.HIRE_DATE).getFullYear() >= 1994));
}
// searchByjoiningYear();


//Delete
//Delete employees details name starts with vowel
function deleteEmployee() {
    employeesData = employeesData.filter(employee => !("aeiou".includes(employee.FIRST_NAME.toLowerCase().charAt(0))));

    console.log("after delete = " + employeesData.length);
...
//GroupBy
//Group Employees name  by their hiring year
function groupByHiringYear() {

    let HiringYears = [...new Set(employeesData.map(employee => new Date(employee.HIRE_DATE).getFullYear()))];

    HiringYears.forEach(year => {
        let employeePerYear = employeesData.filter(employee => new Date(employee.HIRE_DATE).getFullYear() == year);
        //Count people
        console.log(employeePerYear.length + " people joined in " + year);

        console.log(employeePerYear.map(employee => employee.FIRST_NAME + " -> " + year))
    })

}
// groupByHiringYear();
function groupBy(){
    console.log(employeesData.reduce((prev, curr) => {
        let distinctKey = curr.DEPARTMENT_ID;
        if(!prev[distinctKey]){
            prev[distinctKey] = [];
        }
        prev[distinctKey].push(curr.FIRST_NAME + "->" + curr.DEPARTMENT_ID);
        return prev;
    }, {}));
...
//update
//update the salary of person to 70,000 who have current salary 17000
function updateSalary(salary, updateSalary) {
...
// updateSalary(17000, 70000);

function updateEmployee(id){
    let updateValue = getEmployeeObject();
    employeesData.splice(employeesData.findIndex(emp => emp.id == id), 1 , updateValue );
    console.log(employeesData);
}
